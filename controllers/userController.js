...
const changePassword = asyncHandler(async(req, res) => {
    ...
    if(user && passwordIsCorrect){
        ...
        await user.save();
        res.status(200).send("Password changed succesful");
    }else{
        res.status(400);
...
<p>This result link is valid for only 30 minutes.</p>
...
//Hash the token then compare to token in the db
...
user.password = password;
await user.save();
//Delete token if password changes
let token = await Token.findOne({userId: user._id});
if(token){
    await token.deleteOne();
}
